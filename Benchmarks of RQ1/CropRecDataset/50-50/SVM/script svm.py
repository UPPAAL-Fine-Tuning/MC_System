...
def modify_and_run_svm(current_c_value, svma_file, xml_file, last_vars_value, trace_file_path):
    ...
    if accuracy_match:
        ...
        print("Error: Unable to find accuracy in SVM output.")
        return None
    
    PmOpt_value, HpOpt_value = extract_opt_values(trace_file_path)
    ...
    for template in root.iter('template'):
        for declaration in template.iter('declaration'):
            ...
            if 'int C' in declaration.text:
                declaration.text = re.sub(r'int C\s*=\s*\d+\.\d+;', f'int Hp = {current_c_value};', declaration.text)
...
def main():
    trace_file = r'C:\Users\SYRINE\Desktop\Crop_recommendation - 50-50\trace.uctr'
    xml_file = r'C:\Users\SYRINE\Desktop\Crop_recommendation - 50-50\modele.xml'
    svma_file = r'C:\Users\SYRINE\Desktop\Crop_recommendation - 50-50\svm 1.py'
    ...
    if C is not None:
        ...
        if C == Cref:
            ...
            print("Extracted acc_opt_obtained value:", PmOpt_value)
            print("Extracted c_opt_obtained value:", Hp_value)
